...
@inline function SpecularLight(ray::SVector{3, Float32}, intersect::SVector{3, Float32}, obj::Object, NumberOfLights::Int32, Lights::CuDeviceVector{Light})
    ...
    for i in 1:NumberOfLights
        ...
        if light.type == Global
            reflectedLight = reflectRay(SVector{3, Float32}(-1.0f0*(light.direction[1]), -1.0f0(light.direction[2]), -1.0f0(light.direction[3])), intersect, obj)#light.direction
            k = k + Float32(max(dot(ray, reflectedLight), 0.0f0)^15) #vecja cifra pomeni manj specular svetlobe
...
function calculateLight(intersect::SVector{3, Float32}, obj::Object, NumberOfObjects::Int32, Objects::CuDeviceVector{Object}, NumberOfLights::Int32, Lights::CuDeviceVector{Light})
    ...
    #if NumberOfLights != 0
    #    factor = 1.0f0 / Float32(NumberOfLights)
    #end
    ...
    for i in 1:NumberOfLights
        ...
        k::Float32 = 0.0f0
        if light.type == Global
            vecLightDir::SVector{3, Float32} = normalize(SVector{3, Float32}(-1.0f0*(light.direction[1]), -1.0f0*(light.direction[2]), -1.0f0*(light.direction[3])))
            k = calcAngle(intersect, normal, vecLightDir, NumberOfObjects, Objects)
        elseif light.type == Point
            vecToLight::SVector{3, Float32} = normalize(SVector{3, Float32}((light.source[1])-intersect[1], (light.source[2])-intersect[2], (light.source[3])-intersect[3]))
            k = calcAngle(intersect, normal, vecToLight, NumberOfObjects, Objects)
        end
        outputColor[1] = cuda_clamp01(outputColor[1] + k * light.color[1])#*factor*
        outputColor[2] = cuda_clamp01(outputColor[2] + k * light.color[2])#*factor*
        outputColor[3] = cuda_clamp01(outputColor[3] + k * light.color[3])#*factor*
    end
...
function glass(ray::SVector{3, Float32}, intersect::SVector{3, Float32}, obj::Object, Objects::CuDeviceVector{Object}, NumberOfObjects::Int32, Lights::CuDeviceVector{Light}, NumberOfLights::Int32)
    ...
    reflectedPoint::SVector{3, Float32} = SVector{3, Float32}(t*reflectDir[1], t*reflectDir[2], t*reflectDir[3])
    ...
    if t != -1.0f0
        ...
        reflectColor = multiplyColors(Objects[index].color, lightFactor1)#lightFactor1
    end
    ...
    outPoint::SVector{3, Float32} = (t2*ingoingDir[1], t2*ingoingDir[2], t2*ingoingDir[3])
    ...
    intersectPoint2::SVector{3, Float32} = SVector{3, Float32}(t3*outgoingDir[1], t3*outgoingDir[2], t3*outgoingDir[3])
...
@inline function mirrorBounceColor(ray::SVector{3, Float32}, intersect::SVector{3, Float32}, obj::Object, Objects::CuDeviceVector{Object}, NumberOfObjects::Int32, Lights::CuDeviceVector{Light}, NumberOfLights::Int32)
    finalColor::MVector{3, Float32} = MVector{3, Float32}(0.0f0, 0.0f0, 0.0f0)
    for bounce in 1:c_bounces
        #zadeli smo ogledalo - objIndex je index objekta ki je ogledalo
        mirrorLightFactor::SVector{3, Float32} = calculateLight(intersect, obj, NumberOfObjects, Objects, NumberOfLights, Lights)
        mirrorColor::SVector{3, Float32} = multiplyColors(obj.color, mirrorLightFactor)
        bounceRay::SVector{3, Float32} = reflectRay(ray, intersect, obj)
        point_index::SVector{2, Float32} = closestIntersection_kernel(bounceRay/800, 800.0, 18000, pointOnObj) 
        intersectObjIndex::Int32 = Int32(point_index[2])
        intersectPt::Float32 = point_index[1]
        if (intersectPt == -1.0f0) #ce odbit zarek ne zadane nicesar vrnemo barvo ozadja + barvo ogledala
            bgMirrorColor::SVector{3, Float32} = SVector{3, Float32}((1.0f0-c_reflectivity)*mirrorColor[1], (1.0f0-c_reflectivity)*mirrorColor[2], (1.0f0-c_reflectivity)*mirrorColor[3])
            return SVector{3, Float32}(cuda_clamp01(c_reflectivity*bg_color[1] + bgMirrorColor[1]), cuda_clamp01(c_reflectivity*bg_color[2] + bgMirrorColor[2]), cuda_clamp01(c_reflectivity*bg_color[3] + bgMirrorColor[3]))
        elseif Objects[intersectObjIndex] != 2
            return SVector{3, Float32}(cuda_clamp01((1.0f0-c_reflectivity)*mirrorColor[1] + c_reflectivity*objectColor[1]), cuda_clamp01((1.0f0-c_reflectivity)*mirrorColor[2] + c_reflectivity*objectColor[2]), cuda_clamp01((1.0f0-c_reflectivity)*mirrorColor[3] + c_reflectivity*objectColor[3]))
            #return SVector{3, Float32}(finalColor)
        elseif Objects[intersectObjIndex] == 2
            objectColor::SVector{3, Float32} = mirrorBounceColor(ray, Objects[], intersectPt, bounces-1, reflectivity, specularIntensity, specularArea, sun)
            ray = bounceRay
            intersect = SVector{3, Float32}(intersectPt*bounceRay[1], intersectPt*bounceRay[2], intersectPt*bounceRay[3])
            obj = Objects[intersectObjIndex]
        end
    end
    lightFactor::SVector{3, Float32} = calculateLight(intersect, obj, NumberOfObjects, Objects, NumberOfLights, Lights)
    finalColor = MVector{3, Float32}(multiplyColors(obj.color, lightFactor))
    specLight::SVector{3, Float32} = SpecularLight(ray, intersect, obj, NumberOfLights, Lights)
    finalColor = cuda_clamp01(finalColor[1] + specLight[1])
    return SVector{3, Float32}(cuda_clamp01(finalColor[1] + specLight[1]), cuda_clamp01(finalColor[2] + specLight[2]), cuda_clamp01(finalColor[3] + specLight[3]))
end
...
function raytrace_kernel(CameraPosition::SVector{3, Float32}, Pixels::CuDeviceMatrix{SVector{3, Float32}}, NumberOfObjects::Int32, Objects::CuDeviceVector{Object}, NumberOfLights::Int32, Lights::CuDeviceVector{Light}, Image::CuDeviceMatrix{SVector{3, Float32}})
    ...
    if i <= W && j <= H
        ...
        if intersect_info[1] == -1.0f0
            Image[j,i] = bg_color # bg_color::SVector{3, Float32}
            return
        end
...
# specular light
const c_specularIntensity = 0.75f0
const c_specularArea = 12.f0
